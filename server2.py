"""
flask_s1.py — Web UI for DFS node 1 (Go node on :9000).

Provides a browser interface for uploading, browsing, and downloading files
stored on the first Go DFS node.
"""
...
app = Flask(__name__)
app.secret_key = os.environ.get("SECRET_KEY", "dfs-s1-secret")

NODE_HOST = os.environ.get("NODE1_HOST", "127.0.0.1")
NODE_PORT = int(os.environ.get("NODE1_PORT", "9000"))
...
if __name__ == "__main__":
    port = int(os.environ.get("S1_PORT", 5001))
    print(f"[s1] Server 1 UI → http://0.0.0.0:{port}  (Go node: {NODE_HOST}:{NODE_PORT})")
